class Solution(object):
	...
	def setProfit(self, k, prices):
		#print 'prices:%r'%prices
		l = len(prices)/2
		self.A = [[0 for col in xrange(0,l)] for row in xrange(0,l)]
		for i in xrange(0,l):
			for j in xrange(i,l):
				self.A[i][j] = prices[2*j+1]-prices[2*i]
				if i < j:
					if self.A[i+1][j] > self.A[i][j]:
						self.A[i][j] = self.A[i+1][j]
					if self.A[i][j-1] > self.A[i][j]:
						self.A[i][j] = self.A[i][j-1]
		#print 'A:%r'%self.A

		self.P = [[0 for col in xrange(0,l+1)] for row in xrange(0,k+1)]
		for col in xrange(0,k+1):
			for row in xrange(0,l+1):
				if col == 0:
					self.P[col][row] = 0
				elif row == 0:
					self.P[col][row] = 0
				else:
					self.P[col][row] = self.P[col-1][row]
					for x in xrange(1,row+1):
						max = self.P[col-1][row-x] + self.A[row-x][row-1]
						if max > self.P[col][row]:
							self.P[col][row] = max
		#print 'P:%r'%self.P

	def maxProfit(self, k, prices):#Best Time to Buy and Sell Stock IV
		if len(prices) == 0:
			return 0
		...
		if k >= l:
			...
			for x in xrange(0,l/2):
				sum = sum + prices[2*x+1] - prices[2*x]
			return sum
		self.setProfit(k, prices)
	...
	def maxProfit(self, k, prices):#Best Time to Buy and Sell Stock IV
		if len(prices) == 0:
			return 0
		[prices,sum] = self.resetPrices(prices)
		l = len(prices)
		if k >= l:
			return sum
		#P[k stage][l length][True?False]
		P = [[[0,0] for col in xrange(0,l+1)] for row in xrange(0,k+1)]
		for i in xrange(0,k+1):
			for j in xrange(0,l+1):
				if i == 0:
					...
					P[i][j][0] = max(P[i][j-1][0] , P[i][j-1][1])
					P[i][j][1] = max(P[i-1][j-1][0] + prices[j-1][2], 
										P[i-1][j-1][1] + prices[j-1][2],
											P[i][j-1][1] + prices[j-1][1] - prices[j-2][1])
		return max(P[k][l][0],P[k][l][1])
