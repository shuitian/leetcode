...
struct RawNodeInfo
{
	RawNodeInfo(int _minHp, int _currentHp):minHp(_minHp),currentHp(_currentHp){};
	~RawNodeInfo(){};
	// 最低生命
	int minHp;
	// 当前生命
...
{
public:
	std::vector<RawNodeInfo> infoList;
	void addRawInfo(int minHp, int currentHp){
		for (int i = 0; i < infoList.size(); ++i)
		{
			RawNodeInfo currentInfo = infoList[i];
			if(minHp == currentInfo.minHp && currentHp <= currentInfo.currentHp){
				return;
			}
			if(minHp > currentInfo.minHp && currentHp - minHp <= currentInfo.currentHp - currentInfo.minHp){
				return;
			}
			if(minHp <= currentInfo.minHp && currentHp - minHp >= currentInfo.currentHp - currentInfo.minHp){
				infoList[i] = RawNodeInfo(minHp, currentHp);
				return;
			}
		}
		infoList.push_back(RawNodeInfo(minHp, currentHp));
	}

	void print(){
		for (int i = 0; i < infoList.size(); ++i)
		{
			cout<<i<<':'<<infoList[i].minHp<<' '<<infoList[i].currentHp<<endl;
		}
		cout<<endl;
	}
...
public:
	int calculateMinimumHP(vector<vector<int>>& dungeon) {
		...
		// key是横纵坐标，value是可选的<最低生命，当前生命>
		std::map<pair<int, int>, NodeInfo> nodeInfos;
		for (int i = 0; i < dungeon.size(); ++i)
		{
			for (int j = 0; j < dungeon[0].size(); ++j)
			{
				// cout<<i<<'-'<<j<<endl;
				nodeInfos[{i,j}] = NodeInfo();
				if(i == 0 && j == 0){
					nodeInfos[{i,j}].addRawInfo(
						1 + (dungeon[0][0] < 0 ? -dungeon[0][0]: 0), 1 + (dungeon[0][0] >= 0 ? dungeon[0][0]: 0)
					);
				}
				int value = dungeon[i][j];
				if (i != 0){
					fillInfo(i, j, value, nodeInfos[{i,j}], nodeInfos[{i-1,j}]);
				}
				if (j != 0){
					fillInfo(i, j, value, nodeInfos[{i,j}], nodeInfos[{i,j-1}]);
				}
			}
		}
		auto a = nodeInfos[{dungeon.size() - 1, dungeon[0].size() - 1}];
		return a.getMinHp();
	}

	void fillInfo(int i, int j, int value, NodeInfo& info, NodeInfo& lastNodeInfo){
		// cout<<i<<'-'<<j<<'-'<<lastNodeInfo.infoList.size()<<endl;
		for (auto _info = lastNodeInfo.infoList.begin(); _info != lastNodeInfo.infoList.end(); ++_info)
		{
			// cout<<_info->minHp;
			// cout<<_info->currentHp;
			int newHp = value + _info->currentHp;
			int newMinHp = _info->minHp;
			if(newHp <= 0){
				newMinHp += (1 - newHp);
				newHp = 1;
			}
		...
		// cout<<i<<'-'<<j<<'-'<<lastNodeInfo.infoList.size()<<'-'<<info.infoList.size()<<endl;
	}
};
